...
<!--query php baru taruk disini-->
<?php
//error_reporting(E_ALL ^ E_WARNING || E_NOTICE); //----untuk menghilangkan error
$id = $_GET['id'];
$load = mysqli_query($con,"SELECT * FROM dokter WHERE id_dokter = '$id'")or die (mysqli_error());
$loader = mysqli_fetch_array($load);
?>
<?php

$querry = mysqli_query($con,"SELECT * FROM kartu_stock_resep");
$kartu_resep = mysqli_fetch_array($querry);
?>
<?php
//error_reporting(E_ALL ^ E_WARNING || E_NOTICE);//------untuk menghilangkan eror
	//$id = $_GET['id'];
...
?><br>
    <div class="row">
	    ...
	    <form action="" method="post" class="form-horizontal" enctype="multipart/form-data">
		    ...
		    <input type="text" class="form-control" name="dokter" value="<?php echo $loader['nama_dokter'];?>" id="dokter">&nbsp;<br>	
		    <a href="index?p=load_dokter" class="btn btn-primary">Pilih Dokter Palembang</a>
		    <span style="color:red"><u><small>*Pilih/Klik Tombol <b>"Pilih Dokter Palembang"</b> jika dokter berasal dari klinik Palembang</small></u></span>
		    </div>
		    <label>Nama</label>
		    <input type="text" class="form-control" name="nama" id="nama">
		    <label>Rs/Klinik</label>
		    <input type="text" class="form-control"value="<?php echo $loader['klinik_dokter'];?>" name="lokasi" id="lokasi">
		    <label>Upload Resep</label>
		    <input type="file" class="form-control" name="upload_resep" >
		    <label>Note</label>
		    <textarea type="textarea" class="form-control" name="note" id="note"></textarea>
	    	</div>
	    	<div class="col-md-5">
	    	<label>Nama Obat</label>
	    	<select class="form-control" name="nama_obat" id="nama_obat" selected="<?php echo $nama_obat?>" onchange="changeValue(this.value)">
	...
	<?php
		if (isset($_POST['tambah'])) {
				...
				$upload_resep = $_FILES['upload_resep']['name'];
				$source = $_FILES['upload_resep']['tmp_name'];
				$folder ='../files/';
				
				move_uploaded_file($source, $folder.$upload_resep);
				$tanggal = date('y-m-d');
				...
				mysqli_query($con,"INSERT INTO detil_keranjang_resep(id_jual,id_resep_obat,dokter,nama,lokasi,upload_resep,note) values('$kodeotomatis','$id_resep_obat','$dokter','$nama','$lokasi','$upload_resep','$note')");
			...
			//else{
				//$update=
				//$id_kartu = $kartu_resep['id_kartu'];
				
			//	mysql_query("insert into kartu_stock_resep(id_resep_obat,tanggal,nama_obat,jumlah_obat,sisa_stok,obat_keluar) values('$id_resep_obat','$tanggal','$nama_obat','$jumlah_obat','$stok','$jumlah')");
			//}
		...
		<meta http-equiv="refresh" content="2; url='index?p=tambah_transaksi_resep_test1'">
		<?php
	}
	?>
	<hr>
	...
	<table class="table table-bordered" >
		...
		<a href="index?p=cancel_resep&id_jual=<?php echo $renpaku['id_jual']?>" class="btn btn-danger">
	...
	<div class="row">
		<div class="col-md-4">
			...
			<form action="" method="post">
				...
				<!--<a href="index?p=tambah_transaksi_test1" class="btn btn-success" name="proses" value="proses"></a>!-->
			</form>
			</div>
		</div>
		<div class="col-md-4">
		...
		<div class="col-md-4">
			<a href="index?p=cetak_nota_resep_test&id_jual=<?php echo $kodeotomatis;?>" target="_blank" class="btn btn-success"><span class="fa fa-print"></span>&nbsp; Cetak Nota</a>
