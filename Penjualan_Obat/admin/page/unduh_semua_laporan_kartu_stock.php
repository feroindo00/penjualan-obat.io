    ...
    function Header()
{
    ...
    //Logo
    $id=$_GET['id'];
    $rtol="select*from kartu_stock where kd_obat = '$id'";
    $rtoll=mysqli_query($con,$rtol);
    $rtolll=mysqli_fetch_array($rtoll);
    $lol="SELECT tanggal FROM kartu_stock where id_kartu='1'";
...
$pdf->SetX(25);
$sukaes='Sukses';
$sql=mysqli_query($con,"SELECT kartu_stock.* FROM kartu_stock where kd_obat='$id'order by tanggal")or die(mysqli_error());
