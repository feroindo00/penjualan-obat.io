...
$querry = mysqli_query($con,"SELECT * FROM kartu_stock");
...
?><br>
    ...
    <form action="" method="post" class="form-horizontal">
    	 ...
    	 <option value='' hidden='true'>Pilih Jenis Obat</option>
          ...
          <?php
			$jsArray .= "prdName['" . $data['nama_obat'] . "'] = {kd_obat:'" . addslashes($data['kd_obat']) . "',
						...
						jenis_obat:'".addslashes($data['jenis_obat'])."'
						};";
		  ?>
           <?php 
       			}
    	...
    	<select class="form-control">
    		...
    		<option value='Strip' name='Strip' id='Strip'>Strip</option>
    		<option value='Tube' name='Tube' id='Tube'>Tube</option>
    		<option value='Botol' name='Botol' id='Botol'>Botol</option>
    		<option value='Blister' name='Blister' id='Blister'>Blister</option>
    		<option value='Pcs' name='Pcs' id='Pcs'>Pcs</option>
    	</select><input type="submit" name="pcs" value="Pilih" class="btn btn-primary"><br>!-->
	...
	<?php
		if (isset($_POST['tambah'])) {
			...
			$kk=mysqli_query($con,"select * from kartu_stock where tanggal='$tanggal' and nama_obat='$nama_obat'");
			$k=mysqli_fetch_array($kk);
			$l=mysqli_num_rows($kk);
			if($l){
				$updatekartu=$k['obat_keluar']+$jumlah;
				mysqli_query($con,"UPDATE kartu_stock set 
	...
	<div class="row">
		...
		<div class="col-md-4">
			<br>
			<br><br>	
			<a href="index?p=cetak_nota1&id=<?php echo $kodeotomatis;?>" class="btn btn-success"><span class="fa fa-print"></span>&nbsp; Cetak Nota</a>
 ...
 <!-- Custom Dropdown select2-->
    ...
    <script src="../vendor/select2/dist/js/select2.min.js"></script>
   	<script type="text/javascript">
		...
		document.getElementById('jenis_obat').value = prdName[nama_obat].jenis_obat;
