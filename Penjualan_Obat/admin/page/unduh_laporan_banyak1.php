    ...
    function Header()
{
    ...
    $this->Ln(5);
    $this->Cell(190,15,'LAPORAN PENJUALAN OBAT TERBANYAK',0,0,'C');
    $this->Ln(5);
...
$pdf->SetX(5);
$sukaes='Sukses';
$sql=mysqli_query($con,"SELECT detil_transaksi.*,sum(jumlah) as jumlah,obat.*,transaksi_jual.* FROM detil_transaksi INNER JOIN obat ON obat.kd_obat = detil_transaksi.kd_obat INNER JOIN transaksi_jual on transaksi_jual.id_jual = detil_transaksi.id_jual where tanggal and tanggal between '$tanggal_awal' and '$tanggal_akhir' group by nama_obat");
...
$pdf->SetFont('Arial','B',10);
$pdf->setFillColor(222,222,222);;
$pdf->CELL(146,9,'Harga Pokok Penjualan(Hpp)',1,0,'C',1);
